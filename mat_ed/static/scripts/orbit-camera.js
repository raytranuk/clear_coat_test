...
OrbitCameraInputMouse.prototype.getMouseWorldPos = function () {
...
OrbitCameraInputMouse.prototype.panEntity = function (screenPoint, targetEntity) {
    var fromWorldPoint = OrbitCameraInputMouse.fromWorldPoint;
    var toWorldPoint = OrbitCameraInputMouse.toWorldPoint;
    var worldDiff = OrbitCameraInputMouse.worldDiff;
    ...
    //worldDiff.sub2(toWorldPoint, fromWorldPoint);
    worldDiff.sub2(fromWorldPoint, toWorldPoint);

    targetEntity.parent.translate(worldDiff);
...
OrbitCameraInputMouse.prototype.onMouseDown = function (event) {
    switch (event.button) {
        case pc.MOUSEBUTTON_LEFT:
            if (this.allowLook) this.lookButtonDown = true;
            break;
        case pc.MOUSEBUTTON_MIDDLE:
        case pc.MOUSEBUTTON_RIGHT:
            if (this.allowPan) this.panButtonDown = true;
            break;
    }
};
...
OrbitCameraInputMouse.prototype.onMouseUp = function (event) {
    switch (event.button) {
        case pc.MOUSEBUTTON_LEFT:
            if (this.orbitCamera._clickedPortEntity || this.orbitCamera._clickedLineEntity )// && this.orbitCamera._hoveredPortEntity)
            {
                this.orbitCamera._onEndConnecting(this.orbitCamera._hoveredPortEntity);
                this.orbitCamera._clickedPortEntity = null;
                this.orbitCamera._clickedLineEntity = null;
            }
            if (this.allowLook) this.lookButtonDown = false;
            break;
        case pc.MOUSEBUTTON_MIDDLE:
        case pc.MOUSEBUTTON_RIGHT:
            if (this.allowPan) this.panButtonDown = false;
            break;
    }
};
...
OrbitCameraInputMouse.prototype.onMouseMove = function (event) {
    ...
    if (this.orbitCamera._clickedPanelEntity)
    {
        this.panEntity(event, this.orbitCamera._clickedPanelEntity);
    }
    else
    {
...
OrbitCameraInputMouse.prototype.onMouseWheel = function (event) {
    ...
    if (this.entity.camera.projection === pc.PROJECTION_ORTHOGRAPHIC) 
    {
        this.entity.camera.orthoHeight -= event.wheel * this.distanceSensitivity;
    }
    else
    {    
...
OrbitCameraInputTouch.prototype.onTouchMove = function (event) {
    ...
    if (touches.length == 1) {
        ...
        if (this.entity.camera.projection === pc.PROJECTION_ORTHOGRAPHIC) 
        {
            this.entity.camera.orthoHeight -= diffInPinchDistance * this.distanceSensitivity * 0.1;
        }
        else
        {
